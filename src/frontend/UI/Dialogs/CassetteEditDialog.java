...
public class CassetteEditDialog extends JDialog {
    ...
    public CassetteEditDialog(Window parent, Integer cassetteId, int videoId) {
        super(parent,
                cassetteId == null ? "Добавить кассету" : "Редактировать кассету",
    ...
    private void buildUI(Window parent) {
        ...
        form.add(new JLabel("Фильм:"));
        form.add(filmCombo);

        form.add(new JLabel("Качество:"));
        form.add(qualityCombo);

        form.add(new JLabel("Спрос:"));
        form.add(cbDemand);
        ...
        // Enter = save (единая точка, без cbDemand hacks)
    ...
    private void save() {
        ...
        try {
            ...
            if (editCassetteId != null) {
                c.setCassetteId(editCassetteId);
            }

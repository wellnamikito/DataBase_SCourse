...
public class VideoDAO extends BaseDAO {
    ...
    public DefaultTableModel getAll() throws SQLException {
        ...
        renameColumns(m, new String[]{
                I18n.t("f.caption"), I18n.t("f.district"), I18n.t("f.address"), I18n.t("f.type"),
                I18n.t("f.phone"), I18n.t("f.licence"), I18n.t("f.time_start"), I18n.t("f.time_end"),
                I18n.t("f.amount"), I18n.t("f.owner")
        });
        return m;
    }
    ...
    public void insert(Video v) throws SQLException {
        executeInsertGetKey(
                ...
                v.getPhone(), v.getLicence(), v.getTimeStart(), v.getTimeEnd(), v.getAmount(), v.getOwnerId());
    }
    ...
    public void update(Video v) throws SQLException {
        executeUpdate(
                ...
                v.getCaption(), v.getDistrictId(), v.getAddress(), v.getType(),
                v.getPhone(), v.getLicence(), v.getTimeStart(), v.getTimeEnd(), v.getAmount(), v.getOwnerId(), v.getVideoId());
    }

    public void delete(int id) throws SQLException { executeUpdate("DELETE FROM Video WHERE VideoID=?", id); }

    public void updateViaView(int videoId, String caption, String address) throws SQLException {
        executeUpdate("UPDATE vw_video_simple SET Caption=?, Address=? WHERE VideoID=?", caption, address, videoId);
    }

    public DefaultTableModel getSimpleView() throws SQLException {
        DefaultTableModel m = executeQuery("SELECT Caption, Address FROM vw_video_simple ORDER BY Caption");
        renameColumns(m, new String[]{I18n.t("f.caption"), I18n.t("f.address")});
        return m;
    }
    ...
    public DefaultTableModel getRevenueChart() throws SQLException {
        return executeQuery(
                "SELECT V.Caption, COALESCE(SUM(R.Price),0) AS Total FROM Video V " +
                        "LEFT JOIN Receipt R ON V.VideoID = R.VideoID GROUP BY V.VideoID, V.Caption ORDER BY Total DESC");
    }

    private Video mapRow(ResultSet rs) throws SQLException {
        Video v = new Video();
        v.setVideoId(rs.getInt("VideoID")); v.setCaption(rs.getString("Caption"));
        v.setDistrictId(rs.getInt("DistrictID")); v.setAddress(rs.getString("Address"));
        v.setType(rs.getString("Type")); v.setPhone(rs.getString("Phone"));
        v.setLicence(rs.getString("Licence")); v.setTimeStart(rs.getInt("TimeStart"));
        v.setTimeEnd(rs.getInt("TimeEnd")); v.setAmount(rs.getInt("Amount"));
        v.setOwnerId(rs.getInt("OwnerId")); return v;
    }
}